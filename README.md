...
本サービスはプランにより音声処理方式が異なります。

- **Free**: Chrome の Web Speech API を使用（Chrome の仕様により、音声データが Google のサーバーで処理される場合があります）
- **Pro**: 音声データを Cloudflare Workers AI（Whisper）へ送信して文字起こしします

当方は音声データの取扱いを最小化し、詳細はプライバシーポリシーに記載します。
